...
import com.example.prueba1integrador.fragment.*
...
class HomeActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnAbrirRail.setOnClickListener {
            if (binding.navigationRail.visibility == View.VISIBLE) cerrarMenuLateral()
            else abrirMenuLateral()
        }

        binding.viewScrim.setOnClickListener { cerrarMenuLateral() }
    }
    ...
    private fun setupNavigation(rol: String) {
        if (rol == "admin") {
            ...
            binding.navigationRail.menu.findItem(R.id.item_cesta)?.isVisible = true
        }
